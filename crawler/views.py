...
def crawl(request):
    ...
    if not depth.isdigit():
        return render(request, 'searcher/error.html', {'error_message':'The depth must be a number!'})
    depth=int(depth)

    if not depth in xrange(1,26):
        return render(request, 'searcher/error.html', {'error_message':'The depth must be between 1 and 25!'})

    if not 'youtube.com' in video_url:
        return render(request, 'searcher/error.html', {'error_message':'Please enter a YouTube video'})
